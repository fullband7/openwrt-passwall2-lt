...
function get_ip_port_from(str)
	local q = api.util.shellquote(str)
	local result_port = sys.exec("echo -n " .. q .. " | sed -n 's/^.*[:#]\\([0-9]*\\)$/\\1/p'")
	local result_ip = sys.exec("__host=" .. q .. ";__varport=" .. api.util.shellquote(result_port) .. ";echo -n ${__host%%${__varport:+[:#]${__varport}*}}")
...
listen %s
	...
	for i, o in ipairs(listens[port]) do
		...
		if o.export ~= "0" then
			sys.call(". /usr/share/passwall2/utils.sh ; add_ip2route " .. api.util.shellquote(o.origin_address) .. " " .. api.util.shellquote(o.export))
		end
