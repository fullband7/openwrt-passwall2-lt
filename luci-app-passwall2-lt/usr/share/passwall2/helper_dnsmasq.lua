...
function copy_instance(var)
	...
	for line in io.lines("/tmp/etc/dnsmasq.conf." .. DEFAULT_DNSMASQ_CFGID) do
		...
		if line:find("conf%-dir=") == 1 then
			...
			if TMP_DNSMASQ_PATH then
				...
				sys.call("cp -r " .. api.util.shellquote(tmp_path) .. "/* " .. api.util.shellquote(TMP_DNSMASQ_PATH) .. "/ 2>/dev/null")
			end
		end
	...
	if TMP_DNSMASQ_PATH then
		sys.call("rm -rf " .. api.util.shellquote(TMP_DNSMASQ_PATH) .. "/*passwall2*")
	end
