...
o = s:taboption("Main", ListValue, "node", "<a style='color: #fb9a05;text-decoration: none;cursor: default'>" .. translate("Node") .. "</a>")
	...
	if #normal_list > 0 then
		...
		if node.protocol == "_shunt" then
			...
			if v then
				...
				o = s:taboption("Shunt", ListValue, vid .. "-main_node", string.format('<a style="color:#fb9a05">%s</a>', translate("Preproxy Node")), translate("Set the node to be used as a pre-proxy. Each rule (including <code>Default</code>) has a separate switch that controls whether this rule uses the pre-proxy or not."))
				...
				o = s:taboption("Shunt", Flag, vid .. "-fakedns", "FakeDNS")
				o:depends("node", v.id)
				...
				m.uci:foreach(appname, "shunt_rules", function(e)
					...
					if id and e.remarks then
						...
						local pt = s:taboption("Shunt", ListValue, vid .. "-".. id .. "_proxy_tag", string.format('* <a style="color:#fb9a05">%s</a>', e.remarks .. " " .. translate("Preproxy")))
						...
						local fakedns_tag = s:taboption("Shunt", Flag, vid .. "-".. id .. "_fakedns", string.format('* <a style="color:#fb9a05">%s</a>', e.remarks .. " " .. "FakeDNS"))
				...
				o = s:taboption("Shunt", ListValue, vid .. "-" .. id, string.format('* <a style="color:#cc0033">%s</a>', translate("Default")))
				o.cfgvalue = get_cfgvalue(v.id, id)
				o.write = get_write(v.id, id)
				o.remove = get_remove(v.id, id)
				o:depends("node", v.id)
				...
				o = s:taboption("Shunt", ListValue, vid .. "-" .. id, string.format('* <a style="color:#cc0033">%s</a>', translate("Default Preproxy")), translate("When using, localhost will connect this node first and then use this node to connect the default node."))
				o.cfgvalue = get_cfgvalue(v.id, id)
				o.write = get_write(v.id, id)
				o.remove = get_remove(v.id, id)
				o:value("", translate("Close (Not use)"))
		...
		tips.cfgvalue = function(t, n)
			return string.format('<a style="color:#cc0033">%s</a>', translate("There are no available nodes, please add a node first."))
		end
...
if (mods:find("REDIRECT") and mods:find("TPROXY")) or (mods:find("nft_redir") and mods:find("nft_tproxy")) then
	o = s:taboption("Main", Flag, "localhost_proxy", translate("Allow Lan Access"), translate("Allow access to localhost services"))
	o.default = "1"
	o.rmempty = false
	...
	local html = string.format([[<div class="cbi-checkbox"><input class="cbi-input-checkbox" type="checkbox" disabled></div><div class="cbi-value-description"><font color="#fb9a05">%s</font></div>]], translate("Missing components, transparent proxy is unavailable."))
