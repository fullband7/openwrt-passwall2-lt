	...
	if node then
		...
		if node.type ~= "sing-box" then
			...
			if run_socks_instance then
				sys.call("/usr/share/" .. appname .. "/app.sh run_socks " .. api.util.shellquote(
					string.format("flag=%s node=%s bind=%s socks_port=%s config_file=%s relay_port=%s",
						...
						new_port,
						config_file,
						(proxy_tag and relay_port) and tostring(relay_port) or ""
						)
					) .. "> /dev/null")
			end
			node = {
		...
		if node.transport == "tcp" and node.tcp_guise == "http" and (node.tcp_guise_http_host or "") ~= "" then
			v2ray_transport = {
				type = "http",
		...
		if node.transport == "ws" then
			v2ray_transport = {
				...
				early_data_header_name = (node.ws_earlyDataHeaderName) and node.ws_earlyDataHeaderName or nil
			}
		end
...
function gen_config_server(node)
	...
	if node.outbound_node then
		...
		if node.outbound_node == "_iface" and node.outbound_node_iface then
			...
			sys.call("mkdir -p " .. api.util.shellquote(api.TMP_IFACE_PATH) .. " && touch " .. api.util.shellquote(api.TMP_IFACE_PATH .. "/" .. node.outbound_node_iface))
		else
	...
	if node then
				...
				elseif _node_id then
					...
					elseif _node.protocol == "_iface" then
						if _node.iface then
							...
							table.insert(outbounds, _outbound)
							rule_outboundTag = _outbound.tag
							sys.call("mkdir -p " .. api.util.shellquote(api.TMP_IFACE_PATH) .. " && touch " .. api.util.shellquote(api.TMP_IFACE_PATH .. "/" .. _node.iface))
						end
					end
				end
		...
		elseif node.protocol == "_iface" then
			if node.iface then
				...
				sys.call("mkdir -p " .. api.util.shellquote(api.TMP_IFACE_PATH) .. " && touch " .. api.util.shellquote(api.TMP_IFACE_PATH .. "/" .. node.iface))
			end
		else
		...
		if api.cacheFileCompareToLogic(CACHE_TEXT_FILE, content) == false then
			...
			if direct_ipset then
				string.gsub(direct_ipset, '[^' .. "," .. ']+', function(w)
					sys.call("ipset -q -F " .. api.util.shellquote(w))
				end)
				local ipset_prefix_name = "passwall2_" .. node_id .. "_"
				local ipset_list = sys.exec("ipset list | grep 'Name: ' | grep " .. api.util.shellquote(ipset_prefix_name) .. " | awk '{print $2}'")
			...
			if direct_nftset then
				string.gsub(direct_nftset, '[^' .. "," .. ']+', function(w)
					...
					if #split > 3 then
						...
						sys.call("nft flush set " .. api.util.shellquote(family) .. " " .. api.util.shellquote(table_name) .. " " .. api.util.shellquote(set_name) .. " 2>/dev/null")
					end
				end)
				local family = "inet"
				local table_name = "passwall2"
				local nftset_prefix_name = "passwall2_" .. node_id .. "_"
				local nftset_list = sys.exec("nft -a list sets | grep -E " .. api.util.shellquote(nftset_prefix_name) .. " | awk -F 'set ' '{print $2}' | awk '{print $1}'")
				string.gsub(nftset_list, '[^' .. "\r\n" .. ']+', function(w)
					sys.call("nft flush set " .. api.util.shellquote(family) .. " " .. api.util.shellquote(table_name) .. " " .. api.util.shellquote(w) .. " 2>/dev/null")
				end)
			end
		end
	end
	...
	if inbounds or outbounds then
		...
		for index, value in ipairs(config.outbounds) do
			if not value["_flag_proxy_tag"] and not value.detour and value["_id"] and value.server and value.server_port and not no_run then
				sys.call("echo " .. api.util.shellquote(value["_id"]) .. " >> " .. api.util.shellquote(api.TMP_PATH .. "/direct_node_list"))
			end
			for k, v in pairs(config.outbounds[index]) do
				if k:find("_") == 1 then
					config.outbounds[index][k] = nil
