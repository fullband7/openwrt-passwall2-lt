	...
	uci:foreach(CONFIG, "user", function(user)
		...
		if enable and tonumber(enable) == 1 then
			...
			if bind_local and tonumber(bind_local) ~= 1 and port then
				local comment = safe_comment(remarks)
				if nft_flag == "0" then
					ipt(string.format('-A PSW2-SERVER -p tcp --dport %s -m comment --comment "%s" -j ACCEPT', port, comment))
					ip6t(string.format('-A PSW2-SERVER -p tcp --dport %s -m comment --comment "%s" -j ACCEPT', port, comment))
					if udp_forward == 1 then
						ipt(string.format('-A PSW2-SERVER -p udp --dport %s -m comment --comment "%s" -j ACCEPT', port, comment))
						ip6t(string.format('-A PSW2-SERVER -p udp --dport %s -m comment --comment "%s" -j ACCEPT', port, comment))
					end
				else
